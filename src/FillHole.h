...
std::vector<std::pair<int, int>> fillHole(TIterator holeBegin, TIterator holeEnd, G& g, const std::vector<std::pair<size_t, size_t>>& prohibitedEdges)
{
	...
	for (TIterator it = holeBegin; it != holeEnd; ++it)
	{
		...
		{
			std::pair<size_t, size_t> p = { (size_t)*it,(size_t)*kIt }, p1 = {(size_t)*kIt, (size_t)*it};
