...
MainWindow::MainWindow() :scale(1.0),numGraphs(4),numVectorizations(2),numPointSets(2) {
	//ui.setupUi(this);
	...
	checkFlow = new QCheckBox;
	checkFlow->setText("Flow");
	connect(checkFlow, &QCheckBox::stateChanged, [&]() {
		imageLabel->flowHidden = !checkFlow->isChecked();
		imageLabel->update();
		});
	checkFlow->setChecked(false);
	...
	{
		checkPointSets.push_back(new QCheckBox);
		checkPointSets.back()->setChecked(false);
		connect(checkPointSets[i], &QCheckBox::stateChanged, [&](int ignore) {
			for (int j = 0; j < numPointSets; ++j)
			{
				imageLabel->pointSetsHidden[j] = !checkPointSets[j]->isChecked();
			}
			imageLabel->update();
			});
		l->addWidget(checkPointSets[i]);
	}
	...
	imageLabel->graphsVertexToComponent.resize(numGraphs);
	...
	imageLabel->pointSetsHidden.resize(numPointSets, true);
	imageLabel->pointSets.resize(numPointSets);
	imageLabel->point_type.resize(numPointSets);
}
...
void MainWindow::setPolys(const std::vector<MyPolyline>& newPolys, std::vector<QColor> colors, bool flow)
{
	imageLabel->setPolys(newPolys,colors, flow);
}

void MainWindow::setPolys2(const std::vector<MyPolyline>& newPolys, std::vector<QColor> colors, bool flow)
{
    imageLabel->setPolys(newPolys,colors, flow);
}
...
void MainWindow::setGraph(std::string s, const G& g)
{
    ...
    std::vector<int> vertex_to_component(boost::num_vertices(g));
    int nComponents = boost::connected_components(g, &vertex_to_component[0]);
    imageLabel->graphsVertexToComponent[idx] = vertex_to_component;
}
...
void MainWindow::setPointSet(std::string s, const std::vector<cv::Point2d>& pts, const std::vector<keypointType>& pts_type)
{
	int idx = pointSetNameToIdx.size();
	pointSetNameToIdx.insert(std::make_pair(s, idx));
	checkPointSets[idx]->setText(QString::fromStdString(s));
	imageLabel->pointSets[idx] = pts;
	imageLabel->point_type[idx] = pts_type;
}
...
void MainWindow::setCovered(const std::map<vertex_descriptor, bool>& covered)
{
	imageLabel->setCovered(covered);
}
...
{
	if (scale < 80000.0 / std::max(origHeight, origHeight))
	{
